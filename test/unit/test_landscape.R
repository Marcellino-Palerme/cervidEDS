...
check_land <- function(land, poly, width, heigth)
{
  ...
  checkEquals('SpatialPolygons', substr(class(land)[1],1,15))
  # check numbers of polygons
  checkEquals(poly, length(land))
  # check numbers of id
  checkEquals(poly, length(getIdsSpatialPolygons(land)))
  
  bbox_land <- bbox(land)
  # check width
  checkEquals(width, bbox_land[1,2] - bbox_land[1,1])
  # check height
  checkEquals(heigth, bbox_land[2,2] - bbox_land[2,1])
}

# nominal case
test.gen_land_nom <- function()
{
...
# float value case
test.gen_land_float <- function()
{
  land <- gen_land(45.23, 545.36, 843.90)
  
  # validate the created land
  check_land(land, 45, 545.36, 843.90)
...
check_land_type <- function(land, poly, width, heigth, nb_type)
{
  ...
  checkEquals(poly, length(land$id_type))
  
  # check if each value of id_type is a type
  is_type = unique(land$id_type %in% 1:nb_type)
  checkEquals(1, length(is_type))
  checkTrue(is_type)
  
}

#nominal case
test.affect_type_nom <- function()
{
...
test.affect_type_more_poly <- function()
{
  land <- gen_land(38, 457, 965)
...
check_nei <- function(land,lt_nei)
{
  ...
  checkEquals(length(land), length(lt_nei))
...
#nominal case
test.get_neighbours_nom <- function()
{
...
#minimal case
test.get_neighbours_min <- function()
{
  land <- gen_land()
  lt_nei <- get_neighbours(land)
  check_nei(land, lt_nei)
...
#High case
test.get_neighbours_hi <- function()
{
...
check_coords <- function(land, id, lt_ccoords)
{
  ...
  checkEquals(length(lt_ccoords[,1]), nb_commun)
}

# nominal case
test.commun_coords_nom <- function()
{
  land = gen_land()
...
test.commun_coords_no <- function()
{
  land = gen_land()
  nei = get_neighbours(land)
  # take a two polygons not neigthbours
  no_nei = c(1:10)[-c(4, nei[[4]])][1]
  lt_ccoords = commun_coords(land, c(4,no_nei))
  # no commun coords
  checkTrue(is.empty(lt_ccoords))
}

# more two lines commun
test.more_two <- function()
{
  # create landscapes with two polygon have several commun lines
  coords = matrix(c(0,0,30,0,30,15,30,30,30,45,30,60,0,60,0,0),8,2,byrow = T)
  p = Polygon(coords)
  coords = matrix(c(30,0,30,15,30,30,30,45,30,60,60,60,60,0,30,0),8,2,byrow = T)
  p1 = Polygon(coords)
  ...
  checkTrue(length(lt_ccoords[,1]) == 5)
...
test.one_point <- function()
{
  # create landscapes with two polygon have one commun point
  coords = matrix(c(0,0,30,0,30,15,30,30,0,30,0,0),6,2,byrow = T)
  p = Polygon(coords)
  coords = matrix(c(30,30,30,45,30,60,60,60,60,30,30,30),6,2,byrow = T)
  p1 = Polygon(coords)
  ps = Polygons(list(p),ID = c(1))
  ps1 = Polygons(list(p1),ID = c(2))
  land = SpatialPolygons(list(ps, ps1))
  ...
  lt_ccoords = commun_coords(land,c(1,2))
  checkTrue(length(lt_ccoords[,1]) == 2)
  check_coords(land, 1, lt_ccoords)
  check_coords(land, 2, lt_ccoords)
}
